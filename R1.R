...
setwd("C:/Users/Digi/Documents/Custom Office Templates/flowers")
flowers_N <- read.table(file = "flower.csv", header = TRUE, sep = ",",stringsAsFactors = TRUE)

flowers_2 <- read.table(file = "flower.txt", header = TRUE, sep = "\t",stringsAsFactors = TRUE)
...
View(flower)
ggplot(data=flower, aes(height,weight))+
  geom_point()+
...
mpg %>% 
  ...
  geom_point(aes(colour= drv, size= trans),alpha= 0.9)+
  geom_smooth(method = "lm", se = F)+
  facet_wrap(~year)+
